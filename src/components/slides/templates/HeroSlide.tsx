...
export function HeroSlide({ content, slideId, editable = false, styleVariant = 0, darkMode = false, titleSizeStep = 0 }: Props) {
    ...
    return (
      <div className="relative flex h-[1080px] w-[1920px] items-center justify-center overflow-hidden" style={{ background: 'var(--color-surface)' }}>
        ...
        <EditableText value={content.title} onChange={(v) => update(slideId, { title: v } as any)} as="h1" editable={editable}
          className="relative z-10 text-center" style={{ fontFamily: 'var(--font-header)', fontSize: stepType('8xl', titleSizeStep), fontWeight: 900, lineHeight: 0.88, color: textColor }} />
      </div>
    )
  }
    ...
    return (
      <div className="relative flex h-[1080px] w-[1920px] overflow-hidden" style={{ background: 'var(--color-surface)' }}>
        ...
        <div className="relative z-10 flex h-full w-full items-end px-20 pb-20">
          <EditableText value={content.title} onChange={(v) => update(slideId, { title: v } as any)} as="h1" editable={editable}
            style={{ fontFamily: 'var(--font-header)', fontSize: stepType('8xl', titleSizeStep), lineHeight: 0.85, color: textColor, fontWeight: 800 }} />
        </div>
      </div>
    )
  }
  ...
  if (styleVariant === 2) {
    return (
      <div className="relative flex h-[1080px] w-[1920px] items-center justify-center overflow-hidden" style={{ background: 'var(--color-surface)' }}>
        {bg}
        <EditableText value={content.title} onChange={(v) => update(slideId, { title: v } as any)} as="h1" editable={editable}
          className="relative z-10 text-center font-black" style={{ fontFamily: 'var(--font-header)', fontSize: stepType('9xl', titleSizeStep), lineHeight: 0.8, color: textColor, opacity: 0.9 }} />
      </div>
    )
  }
  ...
  return (
    <div className="relative flex h-[1080px] w-[1920px] flex-col overflow-hidden" style={{ background: 'var(--color-surface)' }}>
      ...
      <div className="relative z-10 flex flex-1 flex-col px-20 py-20">
        ...
        <EditableText value={content.title} onChange={(v) => update(slideId, { title: v } as any)} as="h1" editable={editable}
          style={{ fontFamily: 'var(--font-header)', fontSize: stepType('7xl', titleSizeStep), lineHeight: 0.9, color: textColor, fontWeight: 800 }} />
      </div>
    </div>
  )
}
